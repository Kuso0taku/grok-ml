# Linear Regression
We have `y = k*x + b` like func and somehow need to make this line as near as possible to every points.

### Simple Way
Just move `k` (angle) and `b` (y-intersection) a little to every points separately. For example:  
#### IN:
- line `p'' = m*r + b` (p'' like "p with hat"), where `p` - price of house, `m` - price of one flat, `b` - base price (with 0 flats).
- point `(r, p)`
#### Algorithm:
...
### Quadratic way (most popular and faster)
...
#### IN:
- the line with angle `m`, y-intersection `b` and equation `p'' = m*r + b`
- point `(r, p)`
- a little `n` (speed of learning)
#### Algorithm:
- add `n*r(p-p'')` to angle `m` equals `m' = m + n*r*(p-p'')` (the line rotates)
- add `n*(p-p'')` to y-intersection `b` equals `b' = b + n*(p-p'')` (it moves the line)
#### OUT:
- line `p'' = m'*r + b'`

### Absolute Way (high errors protect and low speed)
#### IN:
...
- add `nr` to angle `m` equals `m' = m + n*r` (in the same time, if the point on the right from the OY, the line rotates counterclockwise, otherwise - clockwise).
...
### General Algorithm (optional)
- data points: `x(1), x(2), ..., x(m)` every point is `x(i) = (x1(i), x2(i), ..., xn(i))`
- labels: `y1, y2, ..., ym`
- weights: `w1, w2, ..., wn`
- model shift: `b`
#### Quadratic Way (for example)
##### IN:
- model `y'' = w1*x1 + w2*x2 + ... + wn * xn + b`
- point `(x, y)`
- a little `n` (speed of learning)
##### RESULT:
- model `y'' = w'1*x1 + w'2*x2 + ... + wn'*xn + b'` nearer to the point 
##### Algorithm
- add `n*(y-y'')` to y-intersection `b` equals `b' = b + n*(y-y'')`
- for `i = 1, 2, ..., n` add `n*xi*(y-y'')` to weight `wi` equals `wi' = wi + n*ri*(y-y'')`
##### OUT:
- model `y'' = w'1*x1 + w'2*x2 + ... + wn'*xn + b'`
